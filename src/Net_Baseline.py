...
class Net_Baseline(nn.Module):
    def __init__(self):
        ...
        # 第一层卷积块: 提取基础特征
        # Conv: 1 -> 32 channels, Kernel=3, Padding=1 (保持尺寸)
        # 尺寸变化: 28x28 -> 28x28
    ...
    def forward(self, x):
        ...
        # Flatten: 展平操作，将多维特征图拉成一维向量
        x = x.view(-1, 64 * 7 * 7)
        
        # Fully Connected Layers
        x = F.relu(self.fc1(x))
        x = self.fc2(x) # 输出层不需要激活函数，CrossEntropyLoss会自动处理
        return x
...
if __name__ == '__main__':
    ...
    # ==========================================
    # 在你的主程序训练结束后，这样调用：
    # (假设你当前跑的是 Res_Skip 模型)
    evaluate_detailed(model, DEVICE, test_loader, model_name="Net_Baseline")