...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.conv2(out)
        out += residual  # 核心：将输入加回到输出上
        return F.relu(out)

class Net_Res_Skip(nn.Module):
    def __init__(self):
        super(Net_Res_Skip, self).__init__()
        self.conv1 = nn.Conv2d(1, 32, kernel_size=3, padding=1)
        self.res_block1 = ResidualBlock(32) # 插入残差块
        self.pool = nn.MaxPool2d(2, 2)
        
        self.conv2 = nn.Conv2d(32, 64, kernel_size=3, padding=1)
        self.res_block2 = ResidualBlock(64) # 插入残差块
    ...
    def forward(self, x):
        ...
        x = self.res_block1(x) # 经过残差块
        x = self.pool(x)
        
        x = F.relu(self.conv2(x))
        x = self.res_block2(x) # 经过残差块
        x = self.pool(x)
...
if __name__ == '__main__':
    print(f"使用设备: {DEVICE}")
    print("模型结构: Net_Baseline")
    
    # 实例化模型
    model = Net_Res_Skip().to(DEVICE)
    ...
    evaluate_detailed(model, DEVICE, test_loader, model_name="Net_Res_Skip")